...
private fun runTaskFCFS(tasks: List<Task>): ProgramData {
    val info: MutableList<ChartInfo> = mutableListOf(ChartInfo(-1, tasks[0].arrivalTime, 0))
    ...
    info += ChartInfo(tasks[0].pid, currentRunTime, tasks[0].executionTime)
    for (i in 1 until tasks.size) {
        ...
        info.add(ChartInfo(tasks[i].pid, currentRunTime, tasks[i].executionTime))
    }
